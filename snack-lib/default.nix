...
with rec {
  ...
  executable =
    ...
    let
      ...
      spec =
        if pkgs.lib.length specs == 1 then
          pkgs.lib.head specs
        else
          abort "'executable' can only be called on a single executable";
      exe =
        if spec.packageIsExe then buildAsExecutable spec else abort "'executable' called on a library";
    in
    exe.out;
                        ...
                        paths =
                          ...
                          attrValues (
                            mapAttrs (
                              name: value:
                              let
                                suffix = (stringAsChars (x: if x == "." then "/" else x) name) + ".o";
                              in
  ...
  specsFromPackageFile = packagePath: map mkPackageSpec (pkgDescriptionsFromPath packagePath);
