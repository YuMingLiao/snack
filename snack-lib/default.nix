...
with rec {
  ...
  executable = packageFile:
    let
      ...
      spec = if pkgs.lib.length specs == 1 then
        pkgs.lib.head specs
      else
        abort "'executable' can only be called on a single executable";
      exe = if spec.packageIsExe then
        buildAsExecutable spec
      else
        abort "'executable' called on a library";
    in exe.out;

  # Build a package spec as resp. a library and an executable

  buildAsLibrary = pkgSpec: buildLibrary ghcWith (libraryModSpecs pkgSpec);
  ...
  ghcWith = deps: (ghcWithPackages (ps: map (p: ps.${p}) deps));

  # Normal build (libs, exes)

  inferBuild = packageFile: mkPackages (specsFromPackageFile packageFile);

  mkPackages = pkgSpecs:
    writeText "build.json" (builtins.toJSON (builtins.map (pkgSpec:
      if pkgSpec.packageIsExe then {
  ...
  inferGhci = packageFile: mkPackagesGhci (specsFromPackageFile packageFile);

  mkPackagesGhci = pkgSpecs:
    writeText "hpack-ghci-json" (builtins.toJSON (builtins.map (pkgSpec:
      let
        drv = if pkgSpec.packageIsExe then
          ghciWithMain ghcWith (executableMainModSpec pkgSpec)
        else
          ghciWithModules ghcWith (libraryModSpecs pkgSpec);
      in {
  ...
  pkgDescriptionsFromPath = with rec {
    pkgDescriptionsFromFile = packageFile:
      with rec {
        ...
        ext = if pkgs.lib.length components <= 1 then
          abort ("File " ++ packageFile ++ " does not have an extension")
        else
          pkgs.lib.last components;
      ...
      if ext == "nix" then
        ...
        abort ("Unknown extension " ++ ext ++ " of file " ++ packagePath);
    pkgDescriptionsFromDir = packageDir:
      ...
      if hasPackageYaml && hasPackageNix then
        abort "Found both package.yaml and package.nix in ${packageDir}"
      else if !(hasPackageYaml || hasPackageNix) then
        abort "Couldn't find package.yaml or package.nix in ${packageDir}"
      else if hasPackageYaml then
        pkgDescriptionsFromFile "${builtins.toString packageDir}/package.yaml"
      else
        pkgDescriptionsFromFile "${builtins.toString packageDir}/package.nix";
  };

    packagePath:
    with { pathType = pkgs.lib.pathType packagePath; };
    if pathType == "directory" then
      pkgDescriptionsFromDir packagePath
    else if pathType == "regular" then
      pkgDescriptionsFromFile packagePath
    else
      abort "Don't know how to load package path of type ${pathType}";
  ...
  buildHoogle = packagePath:
    let
      concatUnion = lists:
        ...
        in builtins.attrNames union;
      allDeps = concatUnion (map (spec: spec.packageDependencies { })
        (specsFromPackageFile packagePath));
      drv = haskellPackages.hoogleLocal {
        packages = map (p: haskellPackages.${p}) allDeps;
      };
    in writeText "hoogle-json" (builtins.toJSON {
      build_type = "hoogle";
      result = { exe_path = "${drv.out}/bin/hoogle"; };
    });
