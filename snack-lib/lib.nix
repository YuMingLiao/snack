...
with builtins; rec {
  ...
  foldDAG = fld@{ f, empty, elemLabel, reduce, elemChildren }:
    roots:
    (foldDAGRec fld {
      traversed = { };
      elem' = empty;
  ...
  foldDAGRec = fld@{ f, empty, elemLabel, reduce, elemChildren }:
    acc0: roots:
    let
      insert = acc@{ traversed, elem' }:
        elem:
        let
          label = elemLabel elem;
          children = elemChildren elem;
        in if lib.attrsets.hasAttr label traversed then
          acc
        else
          let
            acc' = {
  ...
  dfsDAG = dfs@{ f, empty, elemLabel, reduce, elemChildren, ... }:
    roots:
    (dfsDAGRec dfs {
      traversed = [ ];
      path = [ ];
  ...
  dfsDAGRec = dfs@{ f, empty, elemLabel, reduce, elemChildren, ... }:
    acc0: roots:
    let
      insert = acc@{ traversed, path, elem' }:
        elem:
        let
          label = elemLabel elem;
          children = elemChildren elem;
