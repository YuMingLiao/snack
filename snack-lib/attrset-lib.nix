...
with lib; rec {
  ...
  */
  ...
  frozen = src: 
    if pathExists (src + "/default.nix") 
      then import (src + "/default.nix") 
      else abort "No default.nix generated by nix-freeze-files. Please `nix-freeze-files .` at source directory.";
  freeze = src:
    let source = /. + src; in
